...
@triton.autotune(configs=get_autotune_config(), key=['BLOCK_N'], use_cuda_graph=True)
@triton.jit
...
@triton.autotune(configs=get_autotune_config(), key=['BLOCK_N', 'NUM_TOKEN_BLKS'], use_cuda_graph=True)
@triton.jit
def _fwd_kernel_stage2(out, exp_sums,  #[num_batch, num_q_heads, MAX_NUM_TOKEN_BLKS]
    ...
    rescale_factor = tl.exp2((ml - max_logit)*log2e)  #[NUM_TOKEN_BLKS_POW2]
...
def _fwd_grouped_kernel_stage1(
    ...
    cur_batch_seq_len = tl.load(B_Seqlen + cur_batch)
    if cur_token_blk_in_batch * BLOCK_N > cur_batch_seq_len:
        return
    
    reduce_dtype = Q.dtype.element_ty
    ...
    kv_tkn_blk_end = tl.minimum(kv_tkn_blk_start + BLOCK_N, cur_batch_seq_len.to(tl.int32))
    ...
    for b in range(num_kv_tkn_blks):
        ...
        k_loc = tl.load(kv_loc_ptr + offs_kv_loc)

        #tl.device_print("k_loc", k_loc)
        offs_buf_k = (k_loc[None, :] * stride_buf_kbs + cur_kv_head * stride_buf_kh + offs_d[:, None])
        #load kt [BLOCK_DMODEL, KV_TKN_BLK_SZ]
        k = tl.load(K_Buffer + offs_buf_k, mask=(offs_d[:, None] < BLOCK_DMODEL),other=0.0).to(reduce_dtype)
        #tl.device_print("k", k)

        #qk using tl.dot[BLOCK_H, KV_TKN_BLK_SZ] in loop
        ...
        #calculate p [BLOCK_H, KV_TKN_BLK_SZ]
        ...
        #load v_index from req_to_tokens ie do logical to physical block translation
        v_loc = tl.load(kv_loc_ptr  + offs_kv_loc)

        #load v [KV_TKN_BLK_SZ, BLOCK_DMODEL]
        offs_buf_v = v_loc[:, None] * stride_buf_kbs + cur_kv_head * stride_buf_kh + offs_d[None, :]
        v = tl.load(V_buffer + offs_buf_v, mask= (offs_d[None, :] < BLOCK_DMODEL), other=0.0).to(reduce_dtype)
    ...
    #load k_loc from req_to_tokens ie do logical to physical block translation
    offs_kv_loc = kv_tkn_blk_start + num_kv_tkn_blks*kv_tkn_blk_sz + offs_kv_tkn_blk
    ...
    #qk using tl.dot[BLOCK_H, KV_TKN_BLK_SZ]
    qk = tl.dot(q, k, out_dtype=tl.float32)
    qk *= sm_scale
    if logit_cap > 0:
        qk = logit_cap * tanh(qk / logit_cap)
...
def _decode_grouped_att_m_fwd(
    ...
    #print(f"grouped_att_m_fwd: grid={grid}")
    _fwd_grouped_kernel_stage1[grid](q, k_buffer, v_buffer, sm_scale, Req_to_tokens, B_req_idx, B_Start_Loc,
                                     ...
                                     BLOCK_H=BLOCK_H, logit_cap=logit_cap, KV_TKN_BLK_SZ=64, matrix_instr_nonkdim=16,kpack=2)
...
def _decode_grouped_reduce_fwd(out, token_blk_logits, exp_sums, max_logits, req_to_token, b_req_idx, b_start_loc,
    ...
    #print(f"grouped_reduce_fwd: grid={grid}")
