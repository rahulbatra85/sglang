...
def _fwd_kernel_stage1(
    ...
    k = tl.load(
        K_Buffer + offs_buf_k,
        mask=(offs_n_new[:, None] < cur_batch_end_index) * (offs_d[None, :] < BLOCK_DMODEL),
        other=0.0
    ).to(reduce_dtype)
    ...
    offs_buf_v = v_log[:, None] * stride_buf_vbs 
...
def _decode_att_m_fwd(
    ...
    BLOCK_DMODEL_POW2 = tl.next_power_of_2(BLOCK_DMODEL)
    ...
    grid=(batch, head_num, num_token_blks)
...
def decode_attention_fwd(
    ...
    num_batch, num_q_heads = b_req_idx.shape[0], q.shape[1]
    num_token_blks = triton.cdiv(max_len_in_batch, BLOCK_N)
    exp_sums = tl.zeros((num_batch, num_q_heads, num_token_blks))
    max_logits = tl.zeros((num_batch, num_q_heads, num_token_blks))
